...
function ResultsPanel({ status, result, errorMessage, onClose, onReset, isOpen = true }) {
  ...
  const hasResultBreakdown = Boolean(result && result.breakdown)
          ...
          {status === 'error' && (
            <div className="text-center py-12">
              ...
              <p className="text-white/80 mb-6 drop-shadow-sm">
                {errorMessage || 'An error occurred while calculating departure time.'}
              </p>
              {onReset && (
                <button
                  onClick={onReset}
          ...
          {status === 'success' && result && (
            <div className="space-y-4">
              <div className="text-center">
                <p className="text-sm text-white/80 mb-1 drop-shadow-sm">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-white drop-shadow-sm">
                        {result.breakdown.baseTravelMinutes} minutes
                      </span>
                    </div>
                    ...
                    {result.breakdown.cabBufferMinutes > 0 && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-white drop-shadow-sm">
                          {result.breakdown.cabBufferMinutes} minutes
                        </span>
                      </div>
                    )}

                    {/* Always show weather adjustment (even 0) */}
                   <div className="flex justify-between">
  <span className="text-white/80 drop-shadow-sm">Weather adjustment:</span>
  <span className="font-medium text-white drop-shadow-sm">
    {result.breakdown.weatherExtraMinutes ?? 0} minutes
  </span>
</div>

<div className="text-sm text-white/70 drop-shadow-sm mt-1">
  Weather: {result.breakdown.weatherSummary || 'Weather unavailable'}
</div>


                    {/* Show weather summary line if present */}
                    {result.breakdown.weatherSummary && (
                      <div className="flex justify-between">
                        <span className="text-white/80 drop-shadow-sm">Weather:</span>
                    ...
                    <div className="flex justify-between border-t border-white/20 pt-2 mt-2">
                      ...
                      <span className="font-bold text-blue-300 drop-shadow-md">
                        {result.breakdown.totalMinutes} minutes
                      </span>
                    </div>
                  </div>
                )}
